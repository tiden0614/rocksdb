...
TEST_F(FlushJobTest, Empty) {
  ...
  FlushJob flush_job(
      ...
      *cfd->GetLatestMutableCFOptions(), nullptr /* memtable_id */,
      ...
      kMaxSequenceNumber, snapshot_checker, &job_context, nullptr,
      nullptr, 0,
...
TEST_F(FlushJobTest, FlushMemTablesSingleColumnFamily) {
  ...
  FlushJob flush_job(
      ...
      *cfd->GetLatestMutableCFOptions(), &flush_memtable_id, env_options_,
      ...
      snapshot_checker, &job_context, nullptr, nullptr, 0,
      nullptr, kNoCompression,
...
TEST_F(FlushJobTest, FlushMemtablesMultipleColumnFamilies) {
  ...
  for (auto cfd : all_cfds) {
    ...
    flush_jobs.emplace_back(
        ...
        &job_context, nullptr, nullptr, 0, nullptr, kNoCompression,
        db_options_.statistics.get(), &event_logger, true,
...
TEST_F(FlushJobTest, Snapshots) {
  ...
  FlushJob flush_job(
      ...
      *cfd->GetLatestMutableCFOptions(), nullptr /* memtable_id */,
      ...
      0, nullptr, kNoCompression, db_options_.statistics.get(),
      &event_logger, true, true /* sync_output_directory */, true /* write_manifest */,
      Env::Priority::USER);
  mutex_.Lock();
  flush_job.PickMemTable();
  ASSERT_OK(flush_job.Run());
